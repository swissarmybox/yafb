...
describe('GET /api/user/:id', () => {
  ...
  it('given no cookie, should fail to get user', async () => {
    // Arrange
    // @ts-ignore
    const request = supertest(global['server']);

    // Act
    const res = await request.get('/api/users/1').send();
  ...
  it('given cookie but incorrect role, should fail to get user', async () => {
    // Arrange
    // @ts-ignore
    const request = supertest(global['server']);
  ...
  it('given cookie, correct role but wrong user id, should fail to get user', async () => {
    // Arrange
  ...
  it('given cookie, correct role but non existing user id, should fail to get user', async () => {
    // Arrange
    const email = 'admin@email.com';
    const password = '123456';
    ...
    expect(res2.body.error).toEqual({
      ...
      message: 'Failed to find user with id 2',
      isOperational: true,
    });
  });

  it('given cookie and correct role, should be able to get user', async () => {
    // Arrange
    const email = 'admin@email.com';
    const password = '123456';
